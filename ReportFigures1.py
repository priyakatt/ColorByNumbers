...
#find contours
_,contours_red,hier = cv2.findContours(maskRed, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)

_,contours_g,hier = cv2.findContours(maskBlue, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
_,contours_b,hier = cv2.findContours(maskGreen, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
#cnt=contours[4]
#epsilon = 0.1*cv2.arcLength(cnt,True)
#approx = cv2.approxPolyDP(cnt,epsilon,True)
...
#cnt2 = contours[1]
#draw contours
cv2.drawContours(canvas, contours_red, -1, (0,0,0),1)
cv2.drawContours(canvas, contours_g, -1, (0,0,0),1)
cv2.drawContours(canvas, contours_b, -1, (0,0,0),1)
cv2.imshow('maskContours', canvas)
shape = 0
...
while shape < 2:
    ...
    cv2.imshow('shapecanv', shape_canvas)
    mask_red_shape = cv2.inRange(shape_canvas, (0,0,0),(180,255,255))
    red_shapes_list.append(mask_red_shape)
    shape_canvas[:]=(255,255,255)
    shape = shape +1
    cv2.imshow('shape',mask_red_shape)

cv2.drawContours(canvas, [contours_red[1]], -1, (0,222,255),thickness=-1)

cv2.imshow('colorshape', canvas)
cv2.waitKey(0)
